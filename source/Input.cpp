...
/*
 *  Callback function for handling mouse cursor position changes.
 *  This function is called by GLFW whenever the mouse cursor moves within the window.
 *  @param window The GLFW window that received the event.
 *  @param xPos The new x-coordinate of the mouse cursor (relative to the left edge of the window).
 *  @param yPos The new y-coordinate of the mouse cursor (relative to the top edge of the window).
...
/*
*  Callback function for handling mouse scroll wheel events.
*  This function is called by GLFW whenever the mouse wheel is scrolled.
*  @param window The GLFW window that received the event.
...
void Input::initCallbacks(Window* window) {
	// Call this ONCE at startup, AFTER ImGui_ImplGlfw_InitForOpenGL
	glfwSetCursorPosCallback(window->get(), mouseCursorCallback);
	glfwSetScrollCallback(window->get(), mouseScrollCallback);
}

/*
 *  Polls for input events (keyboard, mouse) and updates the input state.
...
/*
 *  Sets the cursor type for the specified window.
 *  @param window A pointer to the Window object.
...
/*
 *  Gets the current horizontal mouse scroll offset.
 *  @return The horizontal mouse scroll offset.
 */
...
/*
 *  Gets the current vertical mouse scroll offset.
 *  @return The vertical mouse scroll offset.
 */
