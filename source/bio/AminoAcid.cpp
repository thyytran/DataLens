...
// They are statically allocated and held within the AminoAcid::dict, preventing deallocation.
...
// Initialize the static dictionary with the pre-defined AminoAcid objects.
// This allows accessing AminoAcid objects by their full name, 3-letter abbreviation, or 1-letter abbreviation.
...
/*
 *  Checks if the amino acid is polar.
 *  @return True if the amino acid is polar, false otherwise.
 */
...
/*
 *  Checks if the amino acid is basic.
 *  @return True if the amino acid is basic, false otherwise.
 */
...
/*
 *  Checks if the amino acid is acidic.
 *  @return True if the amino acid is acidic, false otherwise.
 */
...
/*
 *  Checks if the amino acid contains sulfur.
 *  This is determined by inspecting the chemical formula.
 *  @return True if the amino acid contains sulfur, false otherwise.
 */
...
const AminoAcid *AminoAcid::set(
	...
	if (!abbr1.empty()) {
		overwriteWarn(abbr1);
		if (abbr1.size() != 1) {
			std::cerr << "Err > Invalid abbr1: must contain exactly 1 character.\n\n";
			return nullptr;
		}
	}
	if (!abbr3.empty()) {
		overwriteWarn(abbr3);
		if (abbr3.size() != 3) {
			std::cerr << "Err > Invalid abbr3: must contain exactly 3 characters.\n\n";
			return nullptr;
		}
	}
	if (!properties.empty() && properties.size() != 3) {
		std::cerr << "Err > Invalid properties string: must contain exactly 3 characters.\n\n";
		return nullptr;
	}
	for (unsigned int i = 0; i < properties.size(); ++i) {
		if (properties[i] != '0' && properties[i] != '1') {
			std::cerr << "Err > Invalid properties string: must only contain '0' and '1'.\n\n";
			return nullptr;
		}
	}
	if (name.empty() && abbr1.empty() && abbr3.empty()) {
		std::cerr << "Err > At least one of [name, abbr1, abbr3] must be provided.\n\n";
		return nullptr;
	}
...
void AminoAcid::showDict() {
	const unsigned int PADDING = 4; // Define a constant for padding between columns
