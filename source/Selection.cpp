...
void Selection::parseQuery(const std::vector<std::string> &query) {
	...
	for (size_t i = 0; i < query.size(); ++i) {
		...
		if (!paramNameIsValid) {
			std::cerr << "Err > Invalid argument: " << query[i] << "\n\n";
			return;
		}
	}
	...
	if (residueArg.size() > 0) {
		for (size_t i = 0; i < residueArg.size(); ++i) {
			if (residueArg[i] != ':' && 
				...
				std::cerr << "Err > Invalid argument: r=" << residueArg << "\n\n";
				return;
			}
		}
			...
			*/
			...
			if (colonPos != 0) {
				...
				try {
					...
					std::cerr << "Err > Invalid start residue\n\n";
					return;
				}
			}
			...
			if (colonPos != residueArg.size() - 1) {
				...
				try {
					...
					std::cerr << "Err > Invalid end residue\n\n";
					return;
				}
			}
	...
	if (chainArg.size() > 0) {
		if (chainArg.size() > 1) {
			std::cerr << "Err > Invalid chain: must be a single character\n\n";
			return;
		}
		newChain = chainArg[0];
	}
