...
public:
	//static Vec3 rotationSpeed; // Radians per second
	//static float rotationAccumulator;

	static void initAutoRotation();
	static void reset();
	...
	static void autoRotate(const Vec3& speedRadianPerSec);
	static void updateRotation(float deltatime);
	static const MoleculeData* getMoleculeData() { return moleculeData; }
	static const std::vector<float>& getAtomSpheres() { return atomSpheres; }
	static const Mat4& getModelMatrix() { return modelMatrix; }
};
